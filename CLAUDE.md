...
The startup script checks for required external dependencies (Node.js, npm, Claude CLI, Git Bash, uv.exe, caddy.exe), initializes directories, opens firewall port 9529, starts Caddy, then launches the NiceGUI app via `uv run main.py`.
...
**Request flow:** Browser → Caddy (0.0.0.0:9529) → Hub UI (127.0.0.1:8080) or Tool subprocess (127.0.0.1:{dynamic port})
...
- **Async throughout** — httpx for HTTP, asyncio for coordination. NiceGUI is built on FastAPI.
...
- **UI** is NiceGUI with a custom design system in `hub_core/ui/theme.py` (CSS variables, Inter font, dark terminal + light card mixed theme).
