...
.drop-zone input{display:none}.icon{font-size:48px;margin-bottom:12px}.text{color:#666;font-size:14px}
...
.preview canvas{max-width:100%;max-height:100%;display:block}.name-input{width:100%;padding:8px 32px 8px 12px;border:1px solid #ddd;border-radius:8px;font-size:14px;margin-bottom:12px}
...
.modal-crop-box{position:absolute;border:2px solid #cba186;cursor:move;box-shadow:0 0 0 9999px rgba(0,0,0,.3)}
...
{name:'圆形头像512px',size:512,type:'circle',defaultName:'圆形头像-512px'},
{name:'IconTools黑芯',size:1024,type:'icon-black',defaultName:'IconTools-黑芯'},
{name:'IconTools白芯',size:1024,type:'icon-white',defaultName:'IconTools-白芯'}
];
let img,cropData={x:0,y:0,size:100},scale=1,canvasData={w:0,h:0};
...
let modalCtx,modalDrag=false,modalRes=false,modalStart={x:0,y:0},tempCrop={};
...
modalCropBox.style.width=tempCrop.size+'px';
modalCropBox.style.height=tempCrop.size+'px';
...
const dx=e.clientX-modalStart.x,dy=e.clientY-modalStart.y;
if(modalDrag){
tempCrop.x=Math.max(0,Math.min(canvasData.w-tempCrop.size,tempCrop.x+dx));
tempCrop.y=Math.max(0,Math.min(canvasData.h-tempCrop.size,tempCrop.y+dy));
}else if(modalRes){
const delta=Math.max(dx,dy);
tempCrop.size=Math.max(30,Math.min(canvasData.w-tempCrop.x,canvasData.h-tempCrop.y,tempCrop.size+delta));
}
modalCropBox.style.left=tempCrop.x+'px';
modalCropBox.style.top=tempCrop.y+'px';
modalCropBox.style.width=tempCrop.size+'px';
